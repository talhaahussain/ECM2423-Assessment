...
'#' indicates a WALL
'-' indicates a SPACE
Format for a node is (row, column), or (row, element in row).
"""
import time
...
def dfs(start: tuple, end: tuple, grid: list, ordering: list): #-> tuple | bool:
    """
    Opens the given file and loads rows of file as maze rows.
    ...
    while stack:
        ...
        if len(solution) > 2:
            if solution[-1] not in find_neighbours(solution[-2], grid, ordering):
                    # A backtrack has occurred; remove backtracked nodes from solution
                    while solution[-1] not in find_neighbours(solution[-2], grid, ordering):
                        solution.pop(-2) # Keep popping until backtracked nodes have been flushed
        if node == end:
            # Maze exit found; stop running
            result_time = time.time() - start_time # Record time 
            return solution, len(visited), result_time, len(solution), ordering
        
        neighbours = find_neighbours(node, grid, ordering)       
...
def dfs_analysis(maze: list): #-> list | bool:
    """
    Allows analysis of DFS with different permutations by running for all 24 configs on current maze.
    ...
    for config in list(perm): # Going through all config permutations
        result = dfs(targets[0], targets[1], maze, config) # Returns in format: (solution, no. explored, time, no. in solution, config)
        if result == False:
            return False
        print("--- %s nodes explored for DFS with configuration %s ---" % (result[1], config))
        print("--- %s seconds for DFS with configuration %s ---" % (result[2], config))
...
def main() -> None:
    ...
    while True:
        ...
        print("Use DFS to solve the maze you've given? (Respond with 0.)")
        print("Find the optimal permutation of neighbour exploration in DFS for your maze? (Respond with 1).\n")
        ...
        if choice_1 == 0:
            ...
            if choice_2 == 0:
                ...
                while True:
                    ...
                    if config in list(perm):
                        ...
                        continue
                targets = check_entrance_exit(maze)
                result = dfs(targets[0], targets[1], maze, config) # Find entrance and exit
                if result == False:
                    print("We are sorry, a solution to your maze could not be found.")
                    return
                print("Solution:")
            ...
            results.sort(key=lambda a:a[3]) # Sort by shortest path
            print("Configuration %s found the shortest solution out of the group, with (%s nodes)." % (results[0][-1], results[0][3]))
            print("Configuration %s found the longest solution out of the group, with (%s nodes)." % (results[-1][-1], results[-1][3]))
            print("Thank you!")
            input('Press ENTER to exit')
            return
            ...
            print("For example, one configuration may lead to the solution being found faster than another, or in fewer nodes, or a more optimal solution, etc.")
            print("This program's DFS implementation allows for the order in which nodes are discovered to be customised.")
            print("This program also includes an analysis tool for your maze, which will run all 24 configurations of DFS on your maze and report back statistics.")
